...
class TestMessageGenerator(unittest.TestCase):
    ...
    def test_unrecognised_line_should_return_nothing(self):
        action_map = { 'MARNEK1': Action({'expect': 'Marnek enters the realm of the dead', 'message': 'Marnek is in skeleton form, shrouded players DPS now'}) }
        unit = MessageGenerator('/path/to/eq', IRRELEVANT_CHARACTER, action_map)
        line = 'this will not be parsed'
        action = unit.action_for(line)
        self.assertIsNone(action)

    def test_basic_full_line_match_returns_message(self):
        action_map = { 'MARNEK1': {'expect': 'Marnek enters the realm of the dead', 'message': 'Marnek is in skeleton form, shrouded players DPS now'} }
        unit = MessageGenerator('/path/to/eq', IRRELEVANT_CHARACTER, action_map)
    ...
    def test_line_depending_on_character_name_when_character_matches(self):
        action_map = { 'SHADOW6': {'expect': 'shadows will consume you, {CHARACTER}', 'message': 'Run away from the raid'} }
        unit = MessageGenerator('/path/to/eq', 'tabashir', action_map)
    ...
    def test_basic_tell(self):
        action_map = { 'TELL': {'message': 'Incoming Tell', 'expect': 'tells you,', 'ignore': ["That'll be", 'I am unable to wake']} }
        unit = MessageGenerator('/path/to/eq', IRRELEVANT_CHARACTER, action_map)
    ...
    def test_mez_returns_background_task(self):
        action_map = { 'MEZ': {'expect': 'has been mesmerized', 'task': {'title': 'Mez', 'message': 'Mesmerize Warning', 'time': 19}} }
